...
final class FieldTests: XCTestCase {
    ...
    @Field(undefinedError: { FieldError.undefined },
    ...
    @Field(5)
    private var fieldWithInitialValue: Int?
    ...
    override func tearDown() {
        ...
        validatedField = nil
        unvalidatedField = nil
        fieldWithInitialValue = nil
    }

    func testNoInitialValueByDefault() {
        // Assert
        XCTAssertNil(validatedField)
    }

    func testSetInitialValueWhenProvided() {
        // Assert
        XCTAssertEqual(5, fieldWithInitialValue)
    }

    func testNotifyValueChangeWhenObserving() {
        // Arrange
        let valueChanges = ["valid test 1", "t", nil, "valid test 2", "valid test 2", ""]
    ...
    func testPublishValueChangeWhenObserving() {
        // Arrange
        let valueChanges = ["valid test 1", "t", nil, "valid test 2", "valid test 2", ""]
        let expectedReceivedValues = ([$validatedField.wrappedValue] + valueChanges).removeDuplicatedNeighbors() // Add initial value
    ...
    func testNotifyErrorChangeWithDynamicErrorWhenObserving() {
        // Arrange
        let valueChanges = ["valid test 1", "t", "invalid test 1", nil, "valid test 2", "valid test 2", "invalid test 2", ""]
        // The second "valid test 2" should not trigger any validation change because the value didn't change
